...
./bin/ycsb run redis -s -P workloads/workloada -p operationcount=100000 -p "redis.host=$(echo "$ADDRESS" | cut -d'@' -f2-)" -p "redis.port=6379" > results/workload_redis_$1.txt
add_miralis_stat_entry "redis_$1"
...
./bin/ycsb run memcached -s -P workloads/workloada -p operationcount=100000 -p "memcached.hosts=$(echo "$ADDRESS" | cut -d'@' -f2-)" > results/workload_memcached_$1.txt
add_miralis_stat_entry  "memcached_$1"
